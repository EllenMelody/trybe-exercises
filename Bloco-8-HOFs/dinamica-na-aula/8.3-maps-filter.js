...
const olderBooks = books.filter((book) => book.releaseYear < 1962).sort((a, b) => a.releaseYear - b.releaseYear)
...
const newOrder = books.filter((book) => book.genre  === 'Ficção Científica' || book.genre === 'Fantasia')
.map((book)=> book.author.name).sort()
// console.log(newOrder);

// 6 - Crie um array com o nome de todos os livros com mais de 60 anos de publicação.
const olderBooks2 = books.filter((book) => book.releaseYear < 1962).map((name) => name.name)
// console.log(olderBooks2);

// 7 - Encontre o nome do livro escrito pela pessoa cujo nome registrado começa com três iniciais.
// Dica: cada inicial termina com um ponto.
const names = books.map((book) => {
  if (book.author.name.split(" ").length >= 3) {
